...
const AdminActivityLogs = () => {
  ...
  const navItems = [
    ...
    { label: 'oob Management', icon: '🗂' },
    { label: 'User Directory', icon: '👥' },
    { label: 'Activity Logs', icon: '🔄' },
    { label: 'Settings', icon: '⚙️' },
  ];

  const filterTabs = ['Security Events', 'oob Moderation', 'User Permissions', 'Billing Records'];

  const logs = [
    {
      initials: 'SA', avatarBg: 'rgba(19,127,236,0.2)', initialsColor: '#137FEC',
      name: 'Sarah Adams', action: 'Approved oob Post',
      target: 'ID: oOB-92831', targetColor: '#137FEC',
      status: 'Success', statusBg: '#DCFCE7', statusColor: '#166534',
      timestamp: '2 mins ago', iconColor: '#94A3B8', isAlert: false,
    },
    {
      initials: 'MC', avatarBg: '#E2E8F0', initialsColor: '#475569',
      name: 'Mike Chen', action: 'Banned User',
      target: 'user_john_doe@email.com', targetColor: '#0F172A',
      status: 'Success', statusBg: '#DCFCE7', statusColor: '#166534',
      timestamp: '15 mins ago', iconColor: '#94A3B8', isAlert: false,
    },
    {
      initials: 'oR', avatarBg: '#FEF3C7', initialsColor: '#D97706',
      name: 'oessica Rivera', action: 'Updated API Settings',
      target: 'System Config', targetColor: '#0F172A',
      status: 'Warning', statusBg: '#FEF3C7', statusColor: '#92400E',
      timestamp: '42 mins ago', iconColor: '#94A3B8', isAlert: false,
    },
    {
      initials: '??', avatarBg: '#FEE2E2', initialsColor: '#DC2626',
      name: 'Unknown Login', action: 'Failed Authentication',
      target: 'IP: 192.168.1.102', targetColor: '#DC2626',
      status: 'Failed', statusBg: '#FEE2E2', statusColor: '#991B1B',
      timestamp: '1 hour ago', iconColor: '#DC2626', isAlert: true,
    },
  ];

  const stats = [
    {
      label: "TODAY'S ACTIONS", iconBg: 'rgba(19,127,236,0.1)', iconColor: '#137FEC',
      value: '154', subText: '+12% from yesterday', subColor: '#16A34A', showArrow: true,
    },
    {
      label: 'SECURITY ALERTS', iconBg: '#FEE2E2', iconColor: '#EF4444',
      value: '3', subText: 'Requires immediate review', subColor: '#DC2626', showArrow: false,
    },
    {
      label: 'TOP MODERATOR', iconBg: '#FEF3C7', iconColor: '#F59E0B',
      value: 'Sarah Adams', subText: '42 moderation actions today', subColor: '#64748B', showArrow: false, isName: true,
    },
  ];
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: '#FFFFFF', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <header style={{
        ...
        padding: '0 40px', height: '65px', background: '#FFFFFF',
        borderBottom: '1px solid #E2E8F0', position: 'sticky', top: 0, zIndex: 100,
      }}>
        {/* Left: Logo + Nav */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '32px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <div style={{ width: '20px', height: '25px', background: '#137FEC', borderRadius: '4px' }} />
            <span style={{ fontWeight: '700', fontSize: '18px', color: '#0F172A', letterSpacing: '-0.27px' }}>oobPortal Admin</span>
          </div>
          <nav style={{ display: 'flex', alignItems: 'center', gap: '36px' }}>
            {['Dashboard', 'oobs', 'Users'].map(l => (
              <a key={l} href="#" style={{ fontSize: '14px', fontWeight: '500', color: '#475569' }}>{l}</a>
            ))}
            <a href="#" style={{ fontSize: '14px', fontWeight: '700', color: '#137FEC', borderBottom: '2px solid #137FEC', paddingBottom: '4px' }}>Logs</a>
          </nav>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '32px' }}>
          <div style={{ display: 'flex', alignItems: 'center', background: '#F1F5F9', borderRadius: '8px', height: '40px', width: '256px' }}>
            ...
            <input
              ...
              style={{ border: 'none', outline: 'none', fontSize: '16px', background: 'transparent', color: '#0F172A', flex: 1, padding: '0 8px', fontFamily: 'Inter,sans-serif' }}
            />
          </div>
          <div style={{ width: '40px', height: '40px', background: '#CBD5E1', border: '2px solid rgba(19,127,236,0.2)', borderRadius: '9999px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', cursor: 'pointer', overflow: 'hidden' }}>👤</div>
        </div>
      </header>
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        ...
        <aside style={{
          ...
          borderRight: '1px solid #E2E8F0', padding: '16px',
        ...
        <main style={{ flex: 1, padding: '31px 32px 32px', display: 'flex', flexDirection: 'column', gap: '24px', background: '#FFFFFF' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button style={{
              ...
              padding: '8px 16px', background: '#137FEC', border: 'none',
              borderRadius: '8px', fontSize: '16px', fontWeight: '700', color: '#fff',
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: '16px' }}>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '4px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', background: '#F1F5F9', borderRadius: '8px', height: '48px' }}>
                  ...
                  <input
                    placeholder="Search by name, action, or IP address..."
                    style={{ border: 'none', outline: 'none', fontSize: '16px', background: 'transparent', color: '#0F172A', flex: 1, padding: '0 8px', fontFamily: 'Inter,sans-serif' }}
                  />
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                ...
                <button style={{
                  display: 'flex', alignItems: 'center', gap: '8px',
                  padding: '0 16px', height: '48px', background: '#F1F5F9',
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', paddingBottom: '8px' }}>
              {filterTabs.map(tab => (
                <button
                  ...
                  style={{
                    background: activeFilter === tab ? 'rgba(19,127,236,0.1)' : '#F1F5F9',
                    color: activeFilter === tab ? '#137FEC' : '#475569',
                    border: activeFilter === tab ? '1px solid rgba(19,127,236,0.2)' : '1px solid transparent',
                  }}
                >
                  {tab}
                </button>
          ...
          <div style={{
            background: '#FFFFFF', border: '1px solid #E2E8F0',
            boxShadow: '0px 1px 2px rgba(0,0,0,0.05)', borderRadius: '12px', overflow: 'hidden',
          }}>
            {/* Table Header */}
            ...
            <div style={{ borderTop: '1px solid #E2E8F0' }}>
              {logs.map((log, i) => (
                <div key={i} className="log-row" style={{ minHeight: '65px' }}>
                  {/* Admin User */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', width: '178px', flexShrink: 0 }}>
                    <div style={{
                      width: '32px', height: '32px', background: log.avatarBg, borderRadius: '9999px',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontSize: '12px', fontWeight: '700', color: log.initialsColor, flexShrink: 0,
                    }}>{log.initials}</div>
                    <span style={{ fontSize: '14px', fontWeight: '700', color: '#0F172A' }}>{log.name}</span>
                  </div>

                  {/* Action */}
                  <div style={{ width: '172px', padding: '22px 24px', fontSize: '14px', color: '#475569', flexShrink: 0 }}>
                    {log.action}
                  </div>

                  {/* Target */}
                  <div style={{ width: '208px', padding: '22px 24px 22px 0', fontSize: '14px', fontWeight: '500', color: log.targetColor, flexShrink: 0 }}>
                    {log.target}
                  </div>
                  ...
                  <div style={{ width: '95px', padding: '22px 24px 22px 0', flexShrink: 0 }}>
                    <span style={{
                      ...
                      background: log.statusBg, borderRadius: '9999px',
                      fontSize: '12px', fontWeight: '700', color: log.statusColor,
                    }}>{log.status}</span>
                  </div>

                  {/* Timestamp */}
                  <div style={{ width: '107px', padding: '22px 24px 22px 0', fontSize: '14px', color: '#64748B', flexShrink: 0 }}>
                    {log.timestamp}
                  </div>

                  {/* Action icon */}
                  <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-end', padding: '0 24px' }}>
                    <button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px', color: log.iconColor, fontSize: '18px' }}>
                      {log.isAlert ? '⚠️' : 'ⓘ'}
                    </button>
                  </div>
                </div>

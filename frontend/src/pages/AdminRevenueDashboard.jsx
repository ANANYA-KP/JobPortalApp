...
const kpis = [
  { label: 'Monthly Recurring Revenue', value: '$124,500', sub: '+12.5% vs last month', subColor: '#16A34A', iconBg: '#DCFCE7', iconColor: '#16A34A', icon: '↗' },
  { label: 'Churn Rate', value: '2.4%', sub: '-0.5% vs last quarter', subColor: '#DC2626', iconBg: '#FEE2E2', iconColor: '#DC2626', icon: '↘' },
  { label: 'Net Revenue (YTD)', value: '$450,200', sub: 'Fiscal year 2024', subColor: '#64748B', iconBg: 'rgba(19,127,236,0.1)', iconColor: '#137FEC', icon: '📊' },
  { label: 'Active Subscriptions', value: '3,842', sub: 'Across all plans', subColor: '#64748B', iconBg: '#F3E8FF', iconColor: '#9333EA', icon: '∞' },
];
...
const navItems = [
  ...
  { icon: '💼', label: 'Jobs Management' },
  { icon: '👥', label: 'Users' },
  { icon: '💰', label: 'Revenue', active: true },
  { icon: '📊', label: 'Reports' },
];
...
const LineChart = () => {
  ...
  const points = mrrData.map((d, i) => ({
    x: pad.l + (i / (mrrData.length - 1)) * chartW,
    y: pad.t + chartH - (d.value / maxVal) * chartH,
  }));
  ...
  return (
    <svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
      ...
      {points.map((p, i) => (
        <circle key={i} cx={p.x} cy={p.y} r="4" fill="#137FEC" stroke="#fff" strokeWidth="2" />
      ))}
    </svg>
  );
};
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", background: '#F6F7F8', minHeight: '100vh', display: 'flex' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .txn-row { display: grid; grid-template-columns: 2fr 1fr 1fr 1fr; gap: 16px; align-items: center; padding: 16px 24px; border-top: 1px solid #F1F5F9; }
      ...
      <aside style={{ width: 220, background: '#FFFFFF', borderRight: '1px solid #E2E8F0', display: 'flex', flexDirection: 'column', minHeight: '100vh', flexShrink: 0 }}>
        {/* Logo */}
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '24px 20px 20px' }}>
          ...
          <div>
            <p style={{ fontWeight: 700, fontSize: 18, color: '#0F172A', lineHeight: '24px', letterSpacing: '-0.5px' }}>oobPortal</p>
        ...
        <div style={{ borderTop: '1px solid #E2E8F0', padding: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#F8FAFC', borderRadius: 12, padding: 8 }}>
        ...
        <main style={{ padding: 32, flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 32 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <p style={{ fontSize: 16, color: '#64748B' }}>Comprehensive overview of oobPortal's financial health.</p>
            </div>
          ...
          <div style={{ display: 'flex', gap: 24 }}>
            ...
            <div className="card" style={{ flex: 2.4, display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              <div style={{ borderTop: '1px solid #F1F5F9', paddingTop: 16, display: 'flex', justifyContent: 'space-between' }}>
                {mrrData.map(d => (
                  <span key={d.month} style={{ fontSize: 10, fontWeight: 700, color: '#94A3B8' }}>{d.month}</span>
                ))}
              </div>
            </div>

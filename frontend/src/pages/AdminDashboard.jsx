      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ad-nav-link {
          ...
          text-align: left; transition: background 0.12s; isolation: isolate;
          position: relative;
        }
        .ad-nav-link:hover { background: rgba(19,127,236,0.08); }
        .ad-nav-link.active {
          background: #137FEC; color: #FFFFFF; font-weight: 600;
          box-shadow: 0px 4px 6px -1px rgba(19,127,236,0.2), 0px 2px 4px -2px rgba(19,127,236,0.2);
        }
        ...
        .btn-new {
          display: flex; align-items: center; gap: 8px;
          padding: 8.5px 16px 9.5px; height: 38px;
        ...
        .btn-approve-all {
          display: flex; justify-content: center; align-items: center;
          padding: 4.5px 12px 5.5px; height: 26px;
          background: #137FEC; border: none; border-radius: 4px;
          font-family: 'Inter', sans-serif; font-weight: 700; font-size: 12px;
      ...
      <header style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'flex-end', gap: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 0 }}>
                <span style={{ fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: 18, letterSpacing: '-0.45px', color: '#0F172A' }}>JobPortal</span>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 16 }}>
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="ad-icon-btn"><svg width="16" height="20" viewBox="0 0 16 20" fill="none"><path d="M8 0C8 0 3 3 3 10v3.5l-2 2v1h14v-1l-2-2V10C13 3 8 0 8 0Z" stroke="#475569" strokeWidth="1.4"/><path d="M6 17c0 1.105.895 2 2 2s2-.895 2-2" stroke="#475569" strokeWidth="1.4"/></svg></button>
            <button className="ad-icon-btn"><svg width="20" height="20" viewBox="0 0 20 20" fill="none"><circle cx="10" cy="10" r="3" stroke="#475569" strokeWidth="1.4"/><path d="M10 2v2M10 16v2M2 10h2M16 10h2M4.22 4.22l1.41 1.41M14.37 14.37l1.41 1.41M4.22 15.78l1.41-1.41M14.37 5.63l1.41-1.41" stroke="#475569" strokeWidth="1.4" strokeLinecap="round"/></svg></button>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <aside style={{ boxSizing: 'border-box', width: 256, flexShrink: 0, background: '#FFFFFF', borderRight: '1px solid #E2E8F0', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: 16, overflow: 'auto' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {navItems.map(item => (
              <button key={item.label} className={`ad-nav-link${activeNav === item.label ? ' active' : ''}`} onClick={() => setActiveNav(item.label)}>
          ...
          <div style={{ paddingTop: 24, borderTop: '1px solid #E2E8F0' }}>
            <div style={{ position: 'relative', width: '100%', height: 153, background: 'rgba(19,127,236,0.05)', borderRadius: 12, overflow: 'hidden' }}>
              ...
              <button style={{ position: 'absolute', left: 16, right: 16, top: 105, height: 32, background: '#137FEC', borderRadius: 8, border: 'none', cursor: 'pointer', fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: 12, color: '#FFFFFF' }}>
                Upgrade Now
              </button>
            </div>
          </div>
        ...
        <main style={{ flex: 1, overflow: 'auto', padding: 32, display: 'flex', flexDirection: 'column', gap: 32 }}>
            ...
            {kpiCards.map((card, i) => (
              <div key={i} className="ad-kpi-card">
                <div style={{ position: 'absolute', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', left: 25, right: 25, top: 25 }}>
                  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: 36, height: 'auto', minHeight: 32, background: card.iconBg, borderRadius: 8, padding: 8 }}>
                    {card.icon}
                  </div>
                  <span style={{ fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: 12, lineHeight: '16px', color: card.deltaColor, background: card.deltaBg, padding: '4px 8px', borderRadius: '9999px' }}>
                    {card.delta}
                  </span>
                </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 298px', gap: 24, position: 'relative', minHeight: 542 }}>
            ...
            <div style={{ boxSizing: 'border-box', background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 12, padding: 24, display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 24 }}>
                ...
                <button style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: 12, color: '#137FEC' }}>View All</button>
              </div>
          ...
          <div style={{ boxSizing: 'border-box', background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 12, overflow: 'hidden' }}>
            ...
            <div style={{ boxSizing: 'border-box', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 24px', background: 'rgba(248,250,252,0.5)', borderBottom: '1px solid #F1F5F9' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="btn-filter-sm">Filter</button>
                <button className="btn-approve-all">Approve All</button>
              </div>
            </div>
            {/* Table */}
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              {pendingJobs.map((job, i) => (
                <div key={i} style={{
                  display: 'grid', gridTemplateColumns: '200px 1fr 190px 210px',
                  borderTop: i > 0 ? '1px solid #F1F5F9' : 'none', alignItems: 'center',
                }}>
                  {/* Company */}
                  <div style={{ padding: '0 0 0 24px', display: 'flex', alignItems: 'center', gap: 12, height: 65 }}>
